---
title: "Course Agenda (Pacific Time) and Links"
---
...
Agenda <- data.frame(read_csv("../docs/R Workshop Detailed Schedule.csv"))
names(Agenda)<-Agenda[1,]
Agenda<-Agenda[-1,]
Agenda0 <- Agenda
...
Agenda$Title<-paste0(Agenda$`Course Segment`,
                     ifelse( ( Agenda$`Course Segment` == Agenda$Topics | Agenda$`Course Segment` %in% not0 | Agenda$Topics %in% not0 ), "", " - "), 
                     ifelse( ( Agenda$`Course Segment` == Agenda$Topics | Agenda$Topics %in% not0 ), "", Agenda$Topics) )
...
#         cc<-paste0('<a href=', bb[[1]][1], '>link text</', bb[[1]][2], '>')
#         aa[i][[1]][ii]<-cc
#       }
#     }
#   }
...
```{r}
program1<-program[grepl(pattern = "Tuesday, September 22, 2020", x = program$Date), ]
...
```{r}
program1<-program[grepl(pattern = "Wednesday, September 23, 2020", x = program$Date), ]
...
```{r}
program1<-program[grepl(pattern = "Thursday, September 24, 2020", x = program$Date), ]
